<template>
	<div class="flex flex-col space-y-2">
		...
		<div class="relative">
			<div class="absolute inset-y-3 left-3 text-gray-500">
				<slot name="icon" />
			</div>

			<div
				v-if="password"
				class="absolute inset-y-3 right-3 text-gray-500 cursor-pointer"
				@click="showPassword()"
			>
			...
			<input
				class="w-full bg-white dark:bg-dark-400 text-slate-700 dark:text-gray-200 border-3 border-gray-200 dark:border-dark-200 transition duration-150 focus:(outline-none border-purple-300 dark:border-purple-300) py-2 pl-3 text-sm rounded-md"
				:class="[
					{
